...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 获取与发送单聊、群组消息(im:message)<br>以应用的身份发消息(im:message:send_as_bot)<br>发送消息V2(im:message:send)<br>**注意事项**：1. **应用身份**发消息<br>* 需申请下面三个权限之一： **获取与发送单聊、群组消息**（im:message）**以应用的身份发消息**（im:message:send_as_bot）**发送消息V2【历史版本】**（im:message:send）<br>2. **用户身份**发消息<br>* 需同时申请以下两个权限：  **获取与发送单聊、群组消息**（im:message）、 **以用户身份发送消息**（im:message.send_as_user）
